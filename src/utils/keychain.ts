...
export function setSecureTokens(accessToken: string, refreshToken: string){
    console.log(`[setSecureTokens] accessToken: ${accessToken}, refreshToken: ${refreshToken}`)
    return Promise.all([
        Keychain.setGenericPassword('accessToken', accessToken, { service: 'accessToken' }),
        Keychain.setGenericPassword('refreshToken', refreshToken, { service: 'refreshToken' })
    ])
}
